...
| `tuning.ipynb` | **Long-running** systematic tuning notebook (Optuna hyperparameter search per tree model + LSTM, threshold optimisation, walk-forward backtest, soft-vote ensemble).  Default budget ~2–6 h on CPU; lower via `SENTISENSE_OPTUNA_TRIALS=25` for a smoke test. | Same dual feature shape as `poc` + `lstm_forecaster`, plus engineered features (lagged log-returns, RSI-14, rolling vol, day-of-week one-hots, top-N source filter). |
