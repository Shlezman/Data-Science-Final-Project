...
| `tuning.ipynb` | **Long-running** systematic tuning notebook with seven model classes (XGBoost, LightGBM, CatBoost, **ElasticNet logistic regression** as interpretable baseline, **LSTM**, **GRU**, **TCN**), Optuna hyperparameter search per model, isotonic probability calibration, nested-CV threshold optimisation, walk-forward backtest, soft-vote ensemble, abstention analysis (acc/coverage trade-off), multi-seed averaging.  Default budget ~3–8 h on CPU; lower via `SENTISENSE_OPTUNA_TRIALS=25` and `SENTISENSE_LSTM_TRIALS=5` for a smoke test. | Same dual feature shape as `poc` + `lstm_forecaster`, plus engineered features (lagged log-returns, RSI-14, rolling vol, day-of-week one-hots, top-N source filter). |
